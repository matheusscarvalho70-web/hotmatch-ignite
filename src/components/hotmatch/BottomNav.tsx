...
export function BottomNav() {
  ...
  return (
    <nav className="pointer-events-none fixed inset-x-0 bottom-0 z-50 flex justify-center pb-[env(safe-area-inset-bottom)] px-4">
      <div className="glass-panel pointer-events-auto mb-4 flex w-[min(26rem,calc(100%-1.5rem))] items-center justify-between rounded-full bg-background/80 px-2 py-2 shadow-2xl backdrop-blur-xl border border-white/10">
        {items.map(({ to, label, icon: Icon }) => {
          ...
          return (
            <Link
              ...
              className="group tap-scale relative flex flex-1 flex-col items-center justify-center gap-1 rounded-2xl px-1 py-1 text-[10px] font-medium text-muted-foreground transition-all duration-300 hover:text-primary [&.active]:text-primary"
            >
